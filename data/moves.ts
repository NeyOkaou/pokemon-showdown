...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	defog: {
		...
		onHit(target, source, move) {
			...
			const removeAll = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge','contagion'];
	...
	mortalspin: {
		...
		onAfterHit(target, pokemon, move) {
			if (!move.hasSheerForce) {
				...
				const sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge','contagion'];
	...
	simplebeam: {
		...
		pp: 15,
		priority: 0,
		flags: { protect: 1, reflectable: 1, mirror: 1, allyanim: 1, metronome: 1, pulse: 1 },
		onTryHit(target) {
	...
	spellcard: {
		...
		onHit(target, source, move) {
			...
			const removeAll = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge','reflect', 'lightscreen'];
			const removeTarget = ['auroraveil', 'safeguard', 'mist', ...removeAll];
			for (const targetCondition of removeTarget) {
				if (target.side.removeSideCondition(targetCondition)) {
					if (!removeAll.includes(targetCondition)) continue;
	...
	contagion: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				...
				} else if (pokemon.hasAbility('overcoat')||pokemon.hasItem('covertcloak')) {
					// do nothing
				} else {
